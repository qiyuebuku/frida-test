...
|---|---|---|---|
| 1 | f | `T("0")` | "0" |
| 2 | a | `N(资金账号)` | 7 位资金账号（正文禁止记录） |
| **3** | **b** | **`R(交易密码)`** | **6 位交易密码（正文禁止记录）** |
| **4** | **c** | **`S("")`** | **恒空** |
| 5 | h | `Q(通讯密码类输入, 空)` | "" |
...
1. 交易 tab → 多账号登录 → 选择券商页 → 搜索框输 `CCZQ`（ASCII 可 input，
   中文不行）→ 点"川财证券"
2. 登录页：账号、密码只从受控密钥注入，禁止写入技能文档、Git 或日志，
...
## 20. 真机密码登录成功、干净 Redroid 超时的纠偏（第二十轮，2026-08-22）

### 20.1 `account_ready` 假阳性与券商模板缺口
- **现象**：干净 Redroid 中账户对象可构造，状态显示 `account_ready=true`，登录
  已进入 `t3s.b` 报文构造，但 `pwd_result` 最终超时；既有交易实例曾经正常。
- **原因**：Redroid 的券商仓库为空，最小 `a1s`（券商名/券商 ID）只能通过早期
  检查，不能替代官方完整券商对象。下游密码登录还依赖营业部、账号类型规则、
  `yybfunc` 和可生成 `r8m` 的网关路由模板。旧实例的数据目录曾隐式提供这些状态。
- **方案**：从真机官方登录链的序列化边界只提取券商公共模板，删除账号、密码、
  token、session 和动态选择状态；把稳定模板版本化进代码，由冷启动代码重建完整
  `fzr/pzr → g6m/r8m` 对象图。健康检查最终以只读资金查询成功为准。
- **适用范围**：同花顺全新 Redroid/AVD、券商库未初始化、仅靠 seed 重建交易实例。

### 20.2 CBAS 不是这次超时的唯一根因
- **现象**：最初看到 CBAS 路径超时；随后强制切到 native 路径仍然超时，而连接层
  可见 `onChannelOk`。
- **原因**：原“Redroid 正好落入 CBAS 死路”的判断证据不足。两条路径都失败且请求
  已到构造层，更符合上游账户/券商路由上下文不完整，而非单一传输通道故障。
- **方案**：保留 CBAS/native 对照开关，但先比较真机与 Redroid 的完整 `g6m` 和
  `r8m` 元数据，再看发送入口、响应帧和业务回调。`onChannelOk` 只记为传输层就绪。
- **适用范围**：同花顺密码登录无回调、切换通信通道仍无改善的场景。

### 20.3 Zygisk DEX-only 下不能依赖 Hook APK assets
- **现象**：券商模板放入 Hook 的 `assets` 后，Redroid 报找不到文件；尝试
  `createPackageContext` 又出现 Hook 包名不存在。
- **原因**：代码通过 Zygisk 以 DEX 注入 `com.hexin.plat.android`，当前 Context 和
  Assets 都属于同花顺；DEX 注入不等于 Hook APK 被系统安装。
- **方案**：小型券商公共模板编译进注入 DEX（例如专用模板类），构建后检查目标类
  确实位于 companion 会发送的 dex 集合；改 dex 后 force-stop 重启目标进程并用版本
  特征确认加载。该修复在本轮已定位并实现，仍需以干净生产实例登录+只读查询闭环验收。
- **适用范围**：同花顺 Zygisk companion/DEX 注入部署；不适用于正常安装 Hook APK
  且可取得其 package context 的 LSPosed 形态。

### 20.4 最小可恢复材料与安全边界
- **现象**：复制旧 `/data` 能让实例看似恢复，但无法保证下一次服务器损坏后可由代码
  重建；直接导出账户对象又容易夹带敏感字段。
- **原因**：旧目录混有稳定券商元数据和一次性会话状态，无法审计，也会掩盖初始化缺口。
- **方案**：Git 只保存券商公共模板和重建代码；交易账号、密码、token 由 GitHub
  Secrets/生产密钥文件注入；探针采用字段白名单，只导出 `broker` 公共结构，不打印
  原始响应。真机用于取证时保持非生产交易角色，避免与生产单会话互踢。
- **适用范围**：同花顺交易实例的 CI/CD、容灾重建、模板升级和真机辅助验证。

### 20.5 下次复现的最短验证顺序
- **现象**：每修一个猜测就完整部署生产，反馈周期长且容易消耗密码错误次数。
- **原因**：缺少真机发现与 Redroid 重建之间的分阶段验收门。
- **方案**：①真机只读探针确认官方 `g6m/r8m` 形状；②本地验证模板脱敏和对象重建；
  ③构建最终 DEX 并检查模板类实际被打包；④全新 Redroid 数据卷注入密钥后只尝试一次
  密码登录；⑤成功后立即做只读资金查询；⑥失败按 §20.1~20.3 分层取证，密码错误
  不自动重试，超时也不等同于密码错误。
- **适用范围**：同花顺密码登录回归、生产发布前验证和服务器灾难恢复演练。

### 20.6 真机调用链实锤：`f2s.o` 的 `a1s` 参数不可为 null（2026-08-22）
- **现象**：Redroid fallback 能进入 `t3s.b`、通道也报告就绪，却始终没有登录回调；
  代码调用 `f2s.o(info, null, q, callback)`。
- **原因**：真机真实 UI 密码登录脱敏探针确认官方调用为
  `f2s.o(g6m, a1s, q3s, callback)`。第二个参数是字段完整的券商对象；传 null 会让
  `w3s.e` 构造 `v3s/r8m` 时缺失显式券商上下文。真机随后完整通过拦截器链并生成
  新 token，证明调用形状而不是密码内容才是本轮关键差异。
- **方案**：fallback 传入账户管理对象关联的完整 `broker`，并增加契约测试禁止
  `info, null, q, callback` 回归。探针只输出字段名、类型、空值和字符串长度，禁止
  输出实际账号、密码、token 或原始报文。
- **适用范围**：同花顺全新 Redroid 上 `t3s.b` 已到达、channel-ok 但密码登录无回调。

### 20.7 `g6m.N()` 必须取 `pzr.x()` 资金账号（2026-08-22）
- **现象**：补齐 `a1s` 后 Redroid 已得到快速回调，但脱敏结构显示 `g6m.a=null`；
  真机官方登录中同字段长度为 7。
- **原因**：复刻代码从 `pzr.d()` 取值，而生产脱敏导出证明该 getter 在新 seed 对象中
  为空；`pzr.x()` 才返回资金账号。builder 字段非空并不代表取了正确账号域。
- **方案**：`g6m$a.N()` 固定使用 `mgr.x()`，为空时在发送前明确失败；契约测试禁止
  密码登录路径重新使用 `mgr.d()`。继续以字段长度/空值对照，不打印账号正文。
- **适用范围**：同花顺密码登录出现快速无关 fail、探针显示 `g6m.a` 为空的场景。

### 20.8 纯净 seed 密码登录成功后的账户物化（2026-08-22，已闭环）
- **现象**：券商真实返回 `onWeituoLoginSuccess`，但 `izr.l=false`，App 仍显示
  `WeituoNoAccountPage`，随后资金查询收到短响应但业务观察者不回调。
- **原因**：`p0s.F(119)` 可以选出 seed manager，不代表该对象已进入 `n0s.i()`
  账户列表。官方 `p2s.z → n2s.d` 会先以账号、账号类型、账户性质、券商 ID 调
  `p0s.g`，再以 `p0s.b` 回退；两者未命中时 `n2s.d` 提前返回，登录成功响应没有
  完成账户字段落地、券商绑定和仓库持久化。
- **方案**：仅在券商明确 `onWeituoLoginSuccess` 后，对 `p0s.F(119)` 已选出的 seed
  manager 重放 `n2s.d` 的字段落地（`Q/D/F/H/N/I/O/E/W/G`）、`a1s.a`、`pzr.V`
  和 `n0s.c`，再调用官方 `izr.u`。禁止仅设置登录布尔值。最终验收必须同时满足
  `funds + positions + today_order` 三项只读查询成功。本轮全新数据卷已达到
  `READY/session_ready/readonly_probe_ready/write_ready=true`。
- **适用范围**：同花顺纯净 Redroid 中密码校验明确成功，但账户仓库尚未物化。

### 20.9 容器内 Hook 热更新被镜像旧 APK 覆盖
- **现象**：ADB `install -r` 新 Hook 后重启容器，新代码特征始终不出现。
- **原因**：启动管理器会从只读镜像 `/opt/ths/ths-hook.apk` 重新安装 Hook，覆盖
  ADB 热装版本。远端构建 APK 的 SHA-256 与设备 `base.apk` 不一致可直接实锤。
- **方案**：需要重启 Android 时必须重建代码层镜像并重建开发容器；仅调试 Java
  Hook 且不重启容器时才可 ADB 热装并重启目标 App。每轮用构建产物、镜像产物和
  设备安装 APK 的哈希确认实际装载版本。
- **适用范围**：镜像启动阶段会重复安装 APK 的 Redroid/Android 容器。
