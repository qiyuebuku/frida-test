...
### 62. 复刻内部请求时字段映射错位（2026-08-19 血泪，两天归因）
- **现象**: hook 复刻 App 登录协议全链通畅（拦截器/CBAS/券商 155ms 业务响应），但被判"客户交易密码错误"；同密码真机 UI 可登。一度怀疑密码有加密预处理
- **根因**: builder 字段放错位——密码放在 `S()`（线上 tag4，实测恒空），`R()`（tag3，券商密码校验位）传了 "0"。字段错位**不报协议错、只报业务错**，极难归因
- **方案**: 反编译只信调用链和签名，不信 setter 语义。在报文序列化终点（返回 byte[] 的静态方法）加探针，dump 业务对象全字段 + 线上字节 hex，让 App 真实走一次（正确凭据 UI 登录零次数消耗）做字段级对照，修 diff 后单次验证（本次一次成功）
- **适用范围**: 一切"镜像 App 内部调用"场景（登录/下单/签名请求）
- **详文**: `mirror-invocation-verification.md`、`case-ths.md` §19

### 63. 账户对象非空/ready=true 被误当成交易可用（2026-08-22）
- **现象**: 最小账户对象已存在，请求也进入序列化，但登录持续无回调超时
- **原因**: 对象缺少券商路由、营业部和账号类型等语义字段；布尔 ready 只覆盖早期门禁
- **方案**: 分层检查对象完整度、注册、路由、连接、业务回调；最终用只读业务查询验收
- **适用范围**: 所有账户/支付 SDK 的反射对象重建；详见 `debugging-methodology.md` §18、`case-ths.md` §20

### 64. Zygisk 注入 DEX 后误读宿主 assets（2026-08-22）
- **现象**: Hook APK 内资源在普通安装测试可读，DEX-only 注入后 FileNotFound，创建 Hook package context 也失败
- **原因**: 注入代码运行在宿主 Context；DEX 存在不代表 Hook APK 已注册为系统 package
- **方案**: 小型非秘密模板编译进 DEX，或使用显式且带版本校验的 sidecar；按最终注入形态验收
- **适用范围**: Zygisk companion、动态 ClassLoader 和 DEX-only 注入；详见 `debugging-methodology.md` §22

### 65. `onChannelOk`/TCP 已连被误当成业务链已通（2026-08-22）
- **现象**: 通道连接成功但业务请求无响应，切换另一通道仍超时
- **原因**: 连接层不校验账户对象、券商路由和业务注册上下文
- **方案**: 把传输就绪与业务就绪分开；对比序列化对象、发送入口、响应帧，再用权威只读接口定性
- **适用范围**: 所有长连接私有协议；同花顺见 `case-ths.md` §20.2

### 66. 复刻多参数入口时把“看似可选”的上下文对象传 null（2026-08-22）
- **现象**: 请求能序列化、连接成功，但业务端永久无回调
- **原因**: 真机实际传入完整上下文对象；null 不在入口报错，却让下游路由对象缺字段
- **方案**: 在真实入口记录每个参数的类型/null/字段形状，复刻时保持调用形状一致；为非 null 参数增加契约测试
- **适用范围**: 登录、支付和多租户 SDK；同花顺 `f2s.o` 详见 `case-ths.md` §20.6
