...
- 后续变化写入 `candidate_update`，不改写旧 `candidate_base`。
...
已有 community 后续吸收新的子方向时追加。

追加发生在 Assignment LLM 裁决成功后：`attach_existing` 指向已有 community 时，系统从本次 `topic_intent` 中抽取 parent / broad / mid / specific / driver / impact / event thread 信号，与 ledger stats 中该 community 已记录的 `future_coverage` 对比；只有出现新增子方向时才追加。

不依赖下一次 RAG / rerank 再次召回该 community 才追加 update。
...
`absorbed` 是本次新增吸收的子方向列表。update 不记录标题、范围、计数字段、成熟度或长摘要。
...
6. 已存在 candidate 如有新增变化，追加 `candidate_update`。
7. 发送给 LLM 前过滤掉已不在 active community 集合中的陈旧 ID。
8. 调用 Assignment LLM。
9. 对 `attach_existing` 的 community 增加 `accepted_count`，并在出现新增子方向时追加短 `candidate_update`。
...
`candidate_update` 不参与 checkpoint 判断。update 数量增加或 update 文本变长，都不能触发 checkpoint；它只作为尾部变化日志存在，避免频繁 update 反复重建 base 前缀。
...
- candidate_ledger.appended_update
...
- candidate_ledger.candidate_append_log_update_count
...
| 用例 | 验证内容 |
|---|---|
| `test_candidate_ledger_uses_single_append_log_without_reordering_or_midstream_updates` | 候选只追加、不重排，变化追加为 update。 |
...
- `candidate_base` / `candidate_update`。
