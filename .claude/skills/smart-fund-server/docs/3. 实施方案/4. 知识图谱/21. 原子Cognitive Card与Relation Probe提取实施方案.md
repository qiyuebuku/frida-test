...
本阶段实施关系优先重构的第一段链路：把 Evidence Chunk 转换为原子 Cognitive Card。每个 Chunk 使用连续的两轮问答：第一轮只提取 Card、Focus Evidence 与 Relation Probe；Card 确定且至少有两个有效结果后，第二轮在完全相同的对话前缀上追加追问，只判断同 Chunk Card 关系。Probe 只定义跨 Chunk 候选搜索方向，正式跨 Chunk 关系仍交给下一阶段召回、筛选和原文核验。
...
- 第一轮只输出经过去重的最终 Card 集合及 Relation Probe，Relation 规则不能参与第一轮 Card 划分；
- 第二轮必须原样复用第一轮 `system + user + assistant` 消息序列，只在末尾追加 Relation 追问，并基于冻结的 Card ID 输出同 Chunk 正关系；
...
```text
Evidence Chunk
  -> Atomic Cognitive Cards + Relation Probes
  -> 同一对话追加 Intra-Chunk Relation 追问
...
flowchart TD
    ...
    G --> F
    F -->|至少两个有效 Card| R[同一对话追加 Relation 追问]
    R --> RV[Relation Contract Validator]
    F -->|零个或一个 Card| H[Stable Card Identity Builder]
    RV --> H
...
- Relation Probe 与 Card 在第一轮请求中生成并随 manifest 持久化，跨 Chunk Relation Discovery 不再额外调用 Probe Planning LLM；
- 同 Chunk 关系通过同一对话的第二轮追问生成；第二轮完整继承第一轮原文和 Card 输出，不重新发送改写后的上下文，也不经过召回、rerank、Summary 初筛和跨 Chunk 原文核验；
...
- 第一轮顶层严格按顺序包含 `cards` 和 `skip_reason`；
- Card 使用按输出顺序连续的局部 ID；
- 每个 Card Summary 非空；
- 每个 Card 至少引用一个已注册 Span Ref；
- 所有 `focus_evidence_refs` 都属于当前 Chunk；
- 一个 Card 不引用同一 Chunk 中另一个原子事件作为自己的焦点证据。
- 第二轮顶层只包含 `relations`；Relation 两端只能引用第一轮最终 Card；`relation_evidence_refs` 必须属于当前 Chunk，并直接证明两端之间的连接语义；
- 同一 Card 对最多一个关系；同一原子事实不能以 `same_event` 关系代替合并。

两个 Validator 分阶段运行。单张 Card 非法时只丢弃该 Card；第二轮只能引用第一轮仍然有效的局部 Card ID。单条 Relation 非法时只丢弃该 Relation，并记录丢弃数量与错误原因。只要仍存在合法 Card，就不能因为局部 Relation 错误重试或丢弃整个 Chunk。

Validator 不负责使用关键词黑名单判断行业语义，也不根据固定例子决定是否应该拆 Card。原子性、过度拆分和 Probe 价值需要通过 LLM 质量回放评估，不能伪装成确定性规则已经解决。

### 5.4 LLM Feedback Repair

只有第一轮顶层 JSON 不可解析、Card 数组缺失或全部 Card 均无法通过基础契约时，才使用现有 LLM feedback repair 机制进行一次修复。局部 Card 或 Relation 错误不触发 LLM 修复，避免把修复变成常规业务兜底。修复输入应包含：
...
Card/Probe 与 Relation 两轮请求均使用模型提供方的中等推理等级。第一轮只负责事件边界、证据闭合和跨 Chunk 搜索方向；第二轮只负责冻结 Card 之间的原文明示关系。职责拆开后仍保持相同模型与推理配置，避免同一对话内切换参数破坏缓存和判断一致性。推理等级属于请求配置并进入 LLM 缓存键，不与其他等级共享结果。
...
Prompt 不发送 `chunk_id`、`chunk_text` 或独立的 `source_title`。`chunk_id` 只保留在请求 metadata 和程序侧业务上下文中，不参与模型理解；标题通过 `<title>` 进入同一原文坐标系。当标题提供正文未重复的主体、因果、结果或范围时，可作为 Card 证据；第二轮在同一对话中仍能读取该标题并判断 Relation。完整 Chunk 仍保留在程序侧，用于生成 offset、校验证据引用和事实接地。相对时间解释规则属于固定 System Prompt；实际发布时间仍放在动态输入中。

### 7.3 输出结构

一个 Chunk 最多返回两个对象：

1. 第一轮字段顺序为 `cards`、`skip_reason`，Card 项中同时包含 Relation Probe；
2. 只有至少两个有效 Card 时才进行第二轮，第二轮只返回 `relations`。

第二轮不是独立重新提问。请求消息必须以第一轮完整消息数组为前缀，随后追加第一轮 assistant JSON 原文和固定 Relation 追问。LLM Provider 必须保留消息原样，不能因为两轮 JSON Schema 不同而把 Schema 文本注入 system prompt；Schema 继续用于响应校验和 JSON mode，但不参与消息构造。
...
生产链路是两轮连续问答，每轮 `max_tokens` 预留 10000。第一轮只为 Cards/Probe 保留输出预算，第二轮只为 Relations 保留输出预算。零 Card 或单 Card 时不请求第二轮。
...
sequenceDiagram
    ...
    CE->>LLM: Stable Card System Prompt + Dynamic Payload
    LLM-->>CE: 0..N Cards + Relation Probes
    CE->>CV: Validate Cards
    alt validation failed
        CV-->>CE: Structured Issues
        CE->>LLM: Feedback Repair
        LLM-->>CE: Repaired Cards And Probes
        CE->>CV: Validate Again
    end
    alt at least two valid Cards
        CE->>LLM: Original Messages + Assistant Cards + Relation Follow-up
        LLM-->>CE: Intra-Chunk Relations Only
        CE->>CV: Validate Relations Against Frozen Cards
    end
...
保留现有受控并发和前缀预热机制；每个 Chunk 的两轮问答占用同一个并发槽，避免同一 Chunk 的 Relation 追问与 Card 输出脱节。单任务返回 Card 列表及其同 Chunk Relations。聚合层负责：
...
| 两轮职责隔离 | 第一轮 Schema 不含 Relations，第二轮 Schema 不含 Cards 和 Probe。 |
| 追问前缀完全一致 | 第二轮消息以第一轮完整消息数组开头，并原样追加第一轮 assistant JSON。 |
| Provider 不改写前缀 | 两轮都关闭 JSON Schema 文本注入，但继续启用 JSON mode 和 Schema 校验。 |
| 单 Card 跳过追问 | 第一轮仅有零个或一个有效 Card 时不调用 Relation LLM。 |
...
| Prompt 前缀稳定 | 动态时间和正文不进入 System Prompt；第二轮不会重新拼装或压缩第一轮前缀。 |
...
| Langfuse 完整链路 | 可以分别看到 Card/Probe 与 Relation 两轮 LLM、校验、PG、Milvus 和清理 observation。 |
...
4. 第一轮只输出去重后的最终 Cards 与 Relation Probe，第二轮在完全相同的对话前缀后追加追问并只输出同 Chunk Relations；
...
15. Langfuse 能分别观察 `kg.atomic_card.extract_cards` 与 `kg.atomic_card.extract_relations`，并确认第二轮完整继承第一轮消息前缀；
