# 原子 Cognitive Card 与同 Chunk Relation 提取实施方案
...
本阶段实施关系优先重构的第一段链路：把 Evidence Chunk 转换为原子 Cognitive Card，并在同一次 LLM 调用中完成同 Chunk Card 关系提取。跨 Chunk 关系仍交给下一阶段的 Relation Probe 召回与原文核验。
...
- 同一次 LLM 输出先给出经过去重的最终 Card 集合，再基于完整 Card 集合输出同 Chunk 正关系；
- Card manifest 和 Focus Evidence 引用写入 PostgreSQL；
...
本阶段完成的标志不是生成 Graph Community，而是形成一批原子、可检索、证据可回溯的 Cognitive Card，并完成同 Chunk 关系建边。跨 Chunk 搜索计划由下一阶段运行时生成。
...
| 跨 Chunk 搜索材料 | 与 Card 提取耦合。 | 第一阶段不生成；跨 Chunk 关系发现沿用 Card Summary 召回流程。 |
| 下游 | 提取后立即进入 Community Assignment。 | 同 Chunk Edge 写入完成后，把 Card 交给跨 Chunk Relation Discovery。 |
| PG 文本职责 | 保存大量 Card 可读内容。 | 保存 manifest、Focus Evidence refs/offsets 和紧凑结构化事实，不保存完整 Chunk/Card 正文或运行时搜索计划。 |
...
- Relation Probe 不属于本阶段产物，由现有跨 Chunk Relation Discovery 在召回前临时生成；
...
Relation Probe 是下一阶段的临时运行对象，不写入 PG 或 Milvus，也不成为永久事实节点。`focus_evidence_refs` 是 PG 中的证据定位信息，不复制到 Milvus。
...
### 6.4 后续 Relation Probe 边界

Relation Probe 不由本阶段生成，也不属于 Card 数据契约。以下角色和质量规则由下一阶段的 `RelationDiscoveryService` 在跨 Chunk 关系发现开始时使用；本节仅用于说明阶段接口边界。
...
一次 Chunk 调用返回一个对象，字段顺序为 `cards`、`relations`、`skip_reason`。

- `cards` 中每项只包含连续局部 ID、原子 Summary 和证据引用；
- `relations` 只包含最终 Card 之间由当前原文支持的正关系，使用局部 ID 引用端点；
...
5. Card 提取请求和输出中不存在 Relation Probe；
6. 所有 focus refs 都能映射回当前 Chunk 原文；Relation refs 均属于对应 Card；
7. Card ID 在重复运行中满足稳定性要求；
8. PG 保存 manifest、focus refs/offsets 和紧凑结构化信息，不保存完整正文或运行时搜索计划；
...
5. Relation Probe 由下一阶段临时生成，不进入 Card schema 和 generator version；
...
9. Relation Probes 不写入 PG 或 Milvus；focus refs 不写入 Milvus；Summary 与 Focus Evidence 不拼接成单一 embedding。
