...
1. 使用当前 Card Summary 召回历史 Card；
2. 同一个 Summary query 分别检索 Milvus Card Summary View 和 Focus Evidence View；
3. 按 Card ID 合并双视图候选；
...
本阶段不生成 Relation Probe，不增加 `kg_relation_probe_planning` LLM 调用，也不修改 Card 提取阶段的职责。
...
跨 Chunk 召回不依赖 `relation_probes` 字段，也不生成临时 Probe 业务对象。
...
| Langfuse stage audit | Trace 中不存在 Probe Planning，候选为空时不存在初筛和核验请求。 |
...
2. 跨 Chunk 服务不调用 `kg_relation_probe_planning`；
3. 跨 Chunk 召回只使用当前 Card Summary query；
