...
- `kg_graph_changed -> kg_graph_community_refresh` 异步消费入口；
- 从变化 Card 和旧 Community 成员出发，沿 active verified Edge 加载完整受影响闭包；
- 使用无向连通分量形成 Community，同时保留 Edge 原始方向；
- 以最小 active Card ID 作为确定性 identity anchor；
- 根据当前 active Edge 正确处理 create、update、merge、split 和 delete；
- 根据成员 Card、Edge 及 Edge content version 生成 `graph_fingerprint`；
- 重建关系语义的 `kg_graph_communities` 当前态表；
- 生成覆盖完整关系子图的自然语言事实报告，并对引用 alias 做结构校验；
- 在事实报告发布后通过独立任务生成条件性预测，允许无推演基础时返回空预测；
- 使用独立 PG 状态、版本和语义同步水位隔离事实报告与预测；
- 分别发布到 `kg_graph_community_reports` 和 `kg_graph_community_projections`；
- Agent 普通语义搜索默认只召回事实报告，未来、情景类查询才增加预测集合；
- `kg_graph_community_refresh -> kg_graph_community_report -> kg_graph_community_projection` 异步任务链路；
- Redis Community 锁、图指纹 CAS、任务投递水位、失败状态和幂等恢复；
- Langfuse 完整输入、输出、模型和 Token 观测；
- 真实 8 个 Community 的 PG / Milvus 全量生成和重复执行幂等验证。

当前真实验收结果：

- 8 个 active Community 均生成事实报告并进入 `ready`；
- 6 个 Community 生成条件性预测并进入 `ready`；
- 2 个缺少可靠推演基础的 Community 进入 `empty`，且 Milvus 不保留预测 target；
- Milvus 中存在 8 个事实报告 target 和 6 个预测 target；
- 全量重复执行不调用 LLM、不增加报告或预测版本。
...
- `market_co_movement`：同一市场对象或直接资产暴露在不同市场载体上的同步或背离，不主张因果或共同驱动。
