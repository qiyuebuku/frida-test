...
- `reasoning_content`：Provider 返回的完整思考过程；逻辑调用包含 JSON 续写或修复时按物理请求阶段保留；
...
| Reasoning Persistence | DeepSeek 返回的思考过程进入标准响应、审计列和完整响应 JSON |
...
| Langfuse Reasoning | Generation output 可通过 Langfuse API 完整读取 reasoning_content |
