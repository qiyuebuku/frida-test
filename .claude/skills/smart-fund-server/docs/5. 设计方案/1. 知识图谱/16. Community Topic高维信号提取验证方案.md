...
| candidate_fit_judgements | 对候选 community 是否能作为父级目录承接当前 intent 的判断。 |
| rejected_candidates | 拒绝候选的原因。 |
| maintenance_hints | 后续 split / merge 提示。 |

一个 `topic_intent` 可以输出多个 assignment。它可能同时挂入多个已有 Community，也可能在候选都不适合时创建多个新的 L0 Community。系统只用 `max_attach` 控制上限，并校验重复目标；不能用“每个 intent 只能创建一个 L0”这类规则压掉 LLM 的业务裁决。
...
候选判断必须使用“目录归档视角”，而不是“候选摘要是否已经包含当前细节”的视角。

```text
attach_parent:
  候选可以作为父级目录承接当前 intent。
  即使当前细节、主体、风险点、供需状态或项目进展是候选中尚未出现的新材料，也应 attach_existing。

related_but_separate:
  候选与当前 intent 相关，但挂入会污染主题边界，应保持不同 L0。

not_related:
  候选与当前 intent 不相关。
```

如果任一候选被判断为 `attach_parent`，Assignment 必须选择 `attach_existing`，不能同时新建平级 L0。

`create_new_l0` 只允许在所有候选都是 `related_but_separate` 或 `not_related` 时输出。此时 LLM 必须说明为什么这些候选不能作为父级目录承接当前 intent。
...
- 校验 `create_new_l0.new_community.title` 是否非空。
- 校验 `create_new_l0` 是否给出了必要层级裁决理由和 future coverage。
- 校验 `candidate_fit_judgements` 是否引用真实候选，且 action 与 fit 判断自洽：如果存在 `attach_parent` 候选，则不能 `create_new_l0`。
- JSON Schema 必须表达 action 条件约束：`attach_existing.new_community=null`，`create_new_l0.community_id=null`。
...
- 不允许使用关键词黑名单、行业白名单或定向场景规则判断标题是否足够宽。
