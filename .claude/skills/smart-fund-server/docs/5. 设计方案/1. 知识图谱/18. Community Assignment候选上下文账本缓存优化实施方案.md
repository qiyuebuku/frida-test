...
```text
候选上下文只追加，不重排、不改写。
当前实验版不发送 candidate_update，只保留稳定 candidate_base。
...
| community 动态内容变化导致前缀失效 | 不把动态变化发送给 Assignment LLM；完整变化留在 community/card/assignment 明细中追溯。 |
...
- 不发送 `candidate_update`。旧 Redis ledger 中的 `candidate_update` 在加载压缩时直接丢弃。
...
append log 中只保留 `candidate_base` 条目。
...
`candidate_update` 暂不进入 Assignment prompt。原因是实测中模型会引用 update 中的历史细节作为归档依据，容易把单笔交易、公司、数字和产品细节误当成长期主题边界。当前实验版只使用 `candidate_base` 的 title / scope / include_rules / exclude_rules / canonical_labels / granularity_note 判断目录边界。
...
6. 过滤掉已不存在的 community ID，保留仍然 active 的历史候选前缀。
7. 组装 prompt，调用 Assignment LLM。
8. 根据 LLM 输出更新 retrieved / accepted 计数；不追加 candidate_update。
...
`candidate_update` 不进入当前 append log，因此 checkpoint 只根据 `candidate_base` 数量和 base 字符数判断。
...
- 候选只追加，不改写。
- 不发送 candidate update，避免历史细节污染归档判断。
...
- 普通新增事实保留在 Cognitive Card / assignment / community metrics 中，不进入 Assignment 候选 prompt。
- 只有 candidate base 过多或 base 字符数超预算时才生成新的 append log。
