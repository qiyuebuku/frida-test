...
- 可执行的检索操作提示，例如 open / expand / refine，而不是业务动作建议。
...
系统不应该给业务动作建议，只能给检索层面的操作提示。例如：

- 建议打开某个 community 的支撑证据；
- 如果 Agent 关心风险，但当前结果没有命中风险信号，可以提示“当前结果未覆盖风险信号，可按风险侧面 refine”；
- 如果结果过散，可以提示“可收窄到某个 community / 行业 / 主体”；
- 如果结果过窄，可以提示“可扩大到父主题或相关 community”；
- 建议查看同 evidence 的前后文。

这些提示必须来自检索结果结构和覆盖情况，不允许假装理解业务方未声明的目标。
...
| Retrieval Operation Hints | 提示 Agent 可以执行的检索操作，例如 open / expand / refine；不输出业务动作建议。 |

检索系统不强制 Agent 接受建议。Agent 可以忽略建议继续自主规划。
...
| 能力 | 说明 |
|---|---|
| search | 根据 Agent 的搜索意图进行多层召回、重排和证据包组装。 |
| open | 根据命中 ID 打开 community、cognitive card 或 chunk 的完整可读上下文。 |
| expand | 从 community 展开到 cards / chunks，或从 chunk 展开前后文。 |
| refine | 基于上一轮结果和新的约束继续检索。 |
