...
from routers._models import FundRankingRequest, PERIODIC_RATE_TYPES, ANNOUNCEMENT_CATS
...
async def fund_company_list():
    """获取基金公司列表"""
    return await safe_call(_utils.client.get_fund_company_list())
...
async def fund_search(
    ...
    """搜索基金（按名称关键词）"""
    return await safe_call(_utils.client.search_fund(keyword, limit))
...
async def fund_detail(fund_code: str):
    ...
    return await safe_call(_utils.client.get_fund_detail(fund_code))
...
async def nav_trend(
    ...
    return await safe_call(_utils.client.get_nav_trend(fund_code, period))


@router.get("/api/fund/{fund_code}/realtime", summary="实时估值走势", tags=["净值走势"])
async def realtime_trend(fund_code: str):
    """获取实时估值分时走势（每分钟更新）"""
    return await safe_call(_utils.client.get_realtime_trend(fund_code))
...
async def periodic_rate(
    ...
    return await safe_call(_utils.client.get_periodic_rate(fund_code, f"{group}PeriodicRate"))
...
async def profit_contribution(
    ...
    """获取收益贡献分析"""
    return await safe_call(_utils.client.get_profit_contribution(fund_code, time_type))
...
async def top10_holdings(fund_code: str):
    """获取前十大持仓"""
    return await safe_call(_utils.client.get_top10_holdings(fund_code))


@router.get("/api/fund/{fund_code}/holdings/overview", summary="持仓概览", tags=["持仓信息"])
async def holding_overview(fund_code: str):
    """获取持仓概览"""
    return await safe_call(_utils.client.get_holding_overview(fund_code))
...
async def asset_allocation(fund_code: str, manager_id: str = ""):
    """获取资产配置"""
    return await safe_call(_utils.client.get_asset_allocation(fund_code, manager_id))


@router.get("/api/fund/{fund_code}/style", summary="投资风格偏好", tags=["持仓信息"])
async def style_preference(fund_code: str):
    """获取投资风格偏好"""
    return await safe_call(_utils.client.get_style_preference(fund_code))


@router.get("/api/fund/{fund_code}/position/dates", summary="持仓回顾日期列表", tags=["持仓信息"])
async def position_dates(fund_code: str):
    """获取持仓回顾可用的季度日期列表"""
    return await safe_call(_utils.client.get_position_dates(fund_code))
...
async def position_detail(
    ...
    """获取指定季度的前十大持仓明细"""
    return await safe_call(_utils.client.get_position_detail(fund_code, end_date))
...
async def holdings_valuation(fund_code: str):
    ...
    return await safe_call(_utils.client.get_holdings_valuation(fund_code))
...
async def holdings_pe_percentile(
    ...
    return await safe_call(_utils.client.get_holdings_valuation_percentile(fund_code, years))
...
async def manager_info(fund_code: str, manager_id: str = Query(..., description="基金经理ID，如 T191488300")):
    """获取基金经理详细信息"""
    return await safe_call(_utils.client.get_manager_info(fund_code, manager_id))
...
async def manager_profile(manager_id: str):
    ...
    return await safe_call(_utils.client.get_manager_profile(manager_id))
...
async def manager_invest_history(manager_id: str):
    ...
    return await safe_call(_utils.client.get_manager_invest_history(manager_id))
...
async def manager_diagnose(manager_id: str):
    ...
    return await safe_call(_utils.client.get_manager_diagnose(manager_id))
...
async def manager_industry_prefer(manager_id: str):
    """获取基金经理行业偏好"""
    return await safe_call(_utils.client.get_manager_industry_prefer(manager_id))


@router.get("/api/manager/{manager_id}/represent_fund", summary="经理代表基金", tags=["基金经理"])
async def manager_represent_fund(manager_id: str):
    """获取基金经理代表基金"""
    return await safe_call(_utils.client.get_manager_represent_fund(manager_id))
...
async def find_similar_funds(
    ...
    """自动发现同赛道基金（基于行业分布相似度）"""
    return await safe_call(_utils.client.find_similar_funds(fund_code, top_n))
...
async def fund_compare(
    ...
    return await safe_call(_utils.client.get_fund_compare_data(fund_codes))
...
async def trade_rule(fund_code: str):
    ...
    return await safe_call(_utils.client.get_trade_rule(fund_code))
...
async def scale_change(fund_code: str):
    ...
    return await safe_call(_utils.client.get_scale_change(fund_code))
...
async def holder_ratio(fund_code: str):
    ...
    return await safe_call(_utils.client.get_holder_ratio(fund_code))
...
async def dividend_history(fund_code: str):
    """获取分红历史和拆分记录"""
    return await safe_call(_utils.client.get_dividend_history(fund_code))
...
async def rsi_indicator(fund_code: str):
    """获取RSI买卖区间指标"""
    return await safe_call(_utils.client.get_rsi_indicator(fund_code))


@router.get("/api/fund/{fund_code}/track", summary="基金追踪", tags=["指标"])
async def fund_track(fund_code: str):
    """获取基金追踪数据"""
    return await safe_call(_utils.client.get_fund_track(fund_code))
...
async def announcements(
    ...
    return await safe_call(_utils.client.get_announcements(fund_code, category, page, page_size))
...
async def fund_news(
    ...
    """获取基金相关资讯"""
    return await safe_call(_utils.client.get_news(fund_code, limit))
...
async def fund_ranking(req: FundRankingRequest = Body(...)):
    ...
    return await safe_call(_utils.client.get_fund_ranking(
        sort_type=sort_type,
        sort=req.sort,
        limit=req.limit,
        offset=req.offset,
