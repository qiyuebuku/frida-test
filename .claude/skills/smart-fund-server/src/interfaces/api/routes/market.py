...
async def nav_technical(fund_code: str):
    ...
    return await safe_call(_utils.ths.get_nav_technical(fund_code))


@router.get("/api/market/overview", summary="A股大盘总览", tags=["决策辅助"])
async def market_overview():
    """A股大盘总览：指数行情 + 涨跌家数 + 成交额 + 资金流向 + 涨跌停 + 大小盘对比"""
    return await safe_call(_utils.aggregator.get_market_overview())
...
async def yesterday_limit():
    ...
    return await safe_call(_utils.eastmoney.get_yesterday_limit_performance())
...
async def stock_changes(
    ...
    return await safe_call(_utils.eastmoney.get_stock_changes(change_type, min(count, 200)))
...
async def sector_ranking(
    ...
    return await safe_call(_utils.sina.get_sector_ranking(sector_type, min(count, 50)))
...
async def dragon_tiger(
    ...
    return await safe_call(_utils.eastmoney.get_dragon_tiger(tab, days, min(count, 50)))
...
async def ths_dragon_tiger(
    ...
    return await safe_call(_utils.ths.get_ths_dragon_tiger(tab, min(count, 50)))
...
async def capital_flow(
    ...
    return await safe_call(_utils.eastmoney.get_capital_flow(tab, min(days, 60)))
...
async def hot_stocks(
    ...
    return await safe_call(_utils.ths.get_hot_stocks(market))
...
async def hot_plate(
    ...
    return await safe_call(_utils.ths.get_hot_plate(plate_type))


@router.get("/api/hotlist/etf", summary="ETF 热榜", tags=["热榜"])
async def hot_etf():
    """获取 ETF 热度排行"""
    return await safe_call(_utils.ths.get_hot_etf())


@router.get("/api/hotlist/futures", summary="期货热榜", tags=["热榜"])
async def hot_futures():
    """获取期货热度排行"""
    return await safe_call(_utils.ths.get_hot_futures())


@router.get("/api/hotlist/bond", summary="可转债热榜", tags=["热榜"])
async def hot_bond():
    """获取可转债热度排行"""
    return await safe_call(_utils.ths.get_hot_bond())


@router.get("/api/hotlist/topics", summary="热榜话题", tags=["热榜"])
async def hot_topics():
    """获取同花顺热榜话题（15条）"""
    return await safe_call(_utils.ths.get_hot_topics())
...
async def hot_posts(
    ...
    """获取热门文章"""
    return await safe_call(_utils.ths.get_hot_posts(page, page_size))
...
async def theme_articles(
    ...
    return await safe_call(_utils.ths.get_theme_articles(theme_id, page, size))


@router.get("/api/flash_news/tabs", summary="快讯分类标签", tags=["热榜"])
async def flash_news_tabs():
    """获取快讯分类标签列表（A股、重要、公告、期货、异动、港股、美股）"""
    return await safe_call(_utils.ths.get_flash_news_tabs())
...
async def flash_news_list(
    ...
    """获取指定分类的快讯列表（每次约20条）"""
    return await safe_call(_utils.ths.get_flash_news_list(tag_id, seq))


@router.get("/api/news_feed", summary="滚动快讯", tags=["热榜"])
async def news_feed(page: int = Query(1, description="页码")):
    """获取滚动快讯（财经要闻，每页20条）"""
    return await safe_call(_utils.ths.get_news_feed(page))
...
async def news_overview(limit: int = Query(10, description="每类新闻的条数限制")):
    ...
    try:
        ...
        important_news, a_stock_news, feed_news = await asyncio.gather(
            _utils.ths.get_flash_news_list(tag_id=62857, seq=0),  # 重要
            _utils.ths.get_flash_news_list(tag_id=21101, seq=0),  # A股
            _utils.ths.get_news_feed(page=1),  # 滚动快讯
...
async def stock_quote(
    ...
    return await safe_call(_utils.tencent.get_stock_quote(code_list))
...
async def stock_kline(
    ...
    return await safe_call(_utils.eastmoney.get_stock_kline(code, period, min(limit, 500)))
...
async def stock_capital_flow(
    ...
    return await safe_call(_utils.eastmoney.get_stock_capital_flow(code, min(days, 120)))
...
async def stock_valuation(
    ...
    """获取个股 PE_TTM/PB 历史数据"""
    data = await safe_call(_utils.eastmoney.get_stock_valuation_history(code, min(years, 10)))
...
async def stock_financial(
    ...
    return await safe_call(_utils.eastmoney.get_stock_financial(code, min(limit, 50)))
