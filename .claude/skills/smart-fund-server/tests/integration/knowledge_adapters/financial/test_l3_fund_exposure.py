"""Integration tests for L3 event-to-fund exposure paths."""
...
def test_l3_returns_event_to_fund_exposure_with_hard_and_explanation_edges() -> None:
    _ensure_tables()
    _cleanup()
    ...
    policy_node = next(node for node in compile_result.nodes if node.node_type == "policy")

    result = asyncio.run(service.find_financial_paths([policy_node.node_id], max_depth=3, limit=30))
    exposure_paths = [
        path for path in result["paths"] if any(item["node_type"] == "fund" for item in path["path"])
    ]

    assert exposure_paths
    best = exposure_paths[0]
    assert best["path_confidence"] > 0
    assert any(edge.relation_type in {"belongs_to", "holds"} for edge in best["hard_score_edges"])
    assert any(edge.relation_type == "affects" for edge in best["explanation_edges"])
