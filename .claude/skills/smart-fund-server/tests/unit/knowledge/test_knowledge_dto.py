...
def test_research_context_command_defaults_are_api_safe() -> None:
    ...
    assert command.adapter_name == "financial"
    assert command.retrieval_mode == "auto"
    assert command.graph_depth == 3
    assert command.max_chars == 5000


def test_source_projection_command_defaults_to_all_sources() -> None:
    command = KnowledgeSourceProjectionCommand()

    assert command.target == "prod"
    assert command.sources is None
    assert command.limit == 100
    assert command.include_skipped is True
