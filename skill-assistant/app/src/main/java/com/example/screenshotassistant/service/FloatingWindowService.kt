...
class FloatingWindowService : Service() {
    ...
    override fun onCreate() {
        ...
        // 后台加载 Skill 命令到悬浮菜单缓存（HttpClient 可能还没创建，需重试）
        serviceScope.launch(Dispatchers.IO) {
            for (attempt in 1..5) {
                if (HttpClient.instance != null) {
                    val ok = FloatingMenuManager.refreshFromServer(this@FloatingWindowService)
                    Log.i("FloatingWindow", "refreshFromServer attempt=$attempt result=$ok")
                    if (ok) break
                } else {
                    Log.w("FloatingWindow", "refreshFromServer attempt=$attempt: HttpClient not ready, waiting...")
                }
    ...
    /**
     * 统一菜单：上层选截屏类型，下层动态显示对应 Skill 命令
     * 选完命令后才开始截屏
     */
    private fun showFirstLevelMenu() {
        val padding = dp(12)
        var selectedCaptureType = "normal"
        ...
        // 截屏方式选择行
        val captureTypes = listOf(
            Triple("normal", "📷", "截图"),
            Triple("long_scroll", "📜", "长截图"),
            Triple("manual_scroll", "✋", "手动长截"),
        )
        val captureRow = LinearLayout(this).apply {
            orientation = LinearLayout.HORIZONTAL
            gravity = Gravity.CENTER_HORIZONTAL
        }
        val captureCells = mutableListOf<View>()
        ...
        fun updateSelectedStyle() {
            captureCells.forEachIndexed { i, cell ->
                val isSelected = captureTypes[i].first == selectedCaptureType
                cell.background = GradientDrawable().apply {
                    setColor(Color.parseColor(if (isSelected) "#6040A0FF" else "#40FFFFFF"))
                    cornerRadius = dp(10).toFloat()
                    if (isSelected) setStroke(dp(1), Color.parseColor("#80A0D0FF"))
                }
            }
        }
        ...
        fun refreshCommands() {
            ...
            val actions = FloatingMenuManager.getActionsForCaptureType(
                this@FloatingWindowService, selectedCaptureType
            )
            val grouped = actions.groupBy { it.skillDisplayName }

            for ((skillName, skillActions) in grouped) {
                commandsContainer.addView(TextView(this).apply {
                    text = "── $skillName ──"
                ...
                for (row in 0 until rows) {
                    ...
                    for (col in 0 until columns) {
                        ...
                        rowLayout.addView(createMenuCell(action.icon, action.displayName, dp(72)) {
                            dismissMenu()
                            FloatingMenuManager.recordUsage(
                                this@FloatingWindowService,
                                action.skillName, action.commandId, selectedCaptureType
                            )
                            startCaptureAndExecute(selectedCaptureType, action.skillName, action.commandId)
                        })
                    }
                    commandsContainer.addView(rowLayout)
                }
            }
        ...
        for ((type, icon, label) in captureTypes) {
            val cell = createMenuCell(icon, label, dp(72)) {
                selectedCaptureType = type
                updateSelectedStyle()
                refreshCommands()
            }
            captureCells.add(cell)
            captureRow.addView(cell)
        }
        ...
        // 命令区
        container.addView(commandsContainer)
     ...
     * 先选好命令，再开始截屏，截完直接执行
     */
    private fun startCaptureAndExecute(captureType: String, skillName: String, commandId: String) {
        val cType = when (captureType) {
            "long_scroll" -> CaptureType.LONG_SCROLL
            "manual_scroll" -> CaptureType.MANUAL_SCROLL
            else -> CaptureType.NORMAL
    ...
    private fun executeSkillAction(action: FloatingAction, bitmap: Bitmap) {
        Log.i("FloatingWindow", "executeSkillAction: action=${action.skillName}/${action.commandId} display=${action.displayName}")
    ...
    private fun executeSkillWithBitmap(skillName: String, commandId: String, bitmap: Bitmap) {
        Log.i("FloatingWindow", "executeSkillWithBitmap: skill=$skillName cmd=$commandId bitmap=${bitmap.width}x${bitmap.height}")
        val httpClient = HttpClient.instance
        if (httpClient == null) {
            Log.e("FloatingWindow", "executeSkillWithBitmap: HttpClient.instance is null!")
            showStatus("✗ 服务未连接", 3000)
            return
        }

        // 读取用户自定义配置（系统提示词 / 输出规则）
        val actionConfig = ActionConfigStore.get(this, commandId)
        val systemPrompt = actionConfig?.systemPrompt
        val rules = actionConfig?.rules
        Log.i("FloatingWindow", "executeSkillWithBitmap config: commandId='$commandId', found=${actionConfig != null}, systemPrompt='${systemPrompt?.take(80)}', rules='${rules?.take(80)}'")
        // 列出所有已配置的 action，帮助排查 ID 不匹配问题
        val allActions = ActionConfigStore.getAll(this)
        Log.i("FloatingWindow", "executeSkillWithBitmap allActions: ${allActions.map { "${it.id}→sp=${it.systemPrompt?.take(20)}" }}")

        showStatus("正在提交任务...", 10000)
        serviceScope.launch(Dispatchers.IO) {
            val base64 = bitmapToBase64(bitmap)
            Log.i("FloatingWindow", "executeSkillWithBitmap: base64Size=${base64.length} (~${base64.length / 1024}KB)")
            ...
            Log.i("FloatingWindow", "executeSkillWithBitmap: result taskId=$taskId")
