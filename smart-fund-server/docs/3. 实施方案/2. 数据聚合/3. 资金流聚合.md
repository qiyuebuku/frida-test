...
更新类型时重新执行代码和类型一致性校验。
目标实现不再用一个采集间隔控制全部数据维度。盘中实时维度按优先级执行：

- `critical`：30 秒；
- `standard`：60 秒；
- `low`：180 秒。

默认新增标的使用 `standard`。日 K、净值、持仓和规模等维度使用独立的日级或低频
策略，不跟随盘中间隔重复采集。
...
立即任务和定时任务对同一个标的使用相同的标的级 Redis 锁，避免同一代码并发采集。
不同标的使用不同锁，可以并行执行。立即任务使用 `force=true`，不受该标的正常间隔
限制。

### 7.2 定时扫描

Watchlist 扫描任务每 15 秒检查一次状态，但是否真正投递由每个标的优先级、市场
状态和各数据维度的更新时间决定：

1. 停用标的直接跳过；
2. 回填模式或从未成功采集的标的立即执行；
3. 增量模式分别判断实时、日级和低频维度是否到期；
4. 只采集到期标的；
5. 数据成功写入后再推进该标的断点；
