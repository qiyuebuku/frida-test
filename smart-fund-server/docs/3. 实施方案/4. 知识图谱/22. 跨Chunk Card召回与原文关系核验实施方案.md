...
GLM-5.3 不支持关闭思考模式。关系候选筛选、原文证据核验和 `same_fact`
门禁统一显式使用最低档 `low`，并分别保留 16,000、32,000 和 16,000 个总输出
Token 上限。该上限同时包含思考与 JSON 正文；缓存版本必须包含思考策略版本，
防止复用切换前仅有 `reasoning_content` 的截断结果。
...
| GLM-5.3 low reasoning | 三类关系裁决均显式开启思考并使用 `low`，且为结构化正文保留足够输出上限。 |
