...
AIClient2API Provider 名称为 `aiclient2api`。外部同时保留 OpenAI-compatible
`/v1/chat/completions` 与 `/v1/responses`，GLM-5.3 上游改为智谱 Coding Plan 原生
OpenAI Responses 端点 `https://open.bigmodel.cn/api/v1/responses`。Chat Completions
调用由 AIClient2API 在内部转换为 Responses，不再绕行 Anthropic Message 协议。
...
AIClient2API `3.3.7.2` 原始转换器曾把显式 `disabled` 处理为省略整个
`thinking` 字段，这对默认开启思考的模型不等价。生产镜像已修复为继续向 Anthropic
上游发送 `thinking: {"type":"disabled"}`，并保留修复前镜像作为回滚点。需要注意，
GLM-5.3 Coding Plan 上游即使收到该字段，仍可能根据任务生成 thinking；关系抽取因此
继续显式使用 `adaptive + low`，而不是依赖无法保证生效的关闭模式。

原生 Responses 适配额外保留以下映射：

- `extra_body.anthropic.thinking.effort` 或顶层 `reasoning_effort` 转为
  `reasoning.effort`；
- Chat `response_format=json_object` 转为 Responses `text.format=json_object`；
- Responses `output[].content[].reasoning_text` 转回 Chat `reasoning_content`；
- 官方 `usage.output_tokens_details.reasoning_tokens` 原样返回，不再依赖字符估算。

Coding Plan Token 调用 `/api/v1/responses` 已验证成功；同一 Token 调用公共 API
`/api/paas/v4/chat/completions` 会进入公共余额计费并返回余额不足，因此生产不得把
Coding Plan Token 配到该 Chat Completions Base URL。
