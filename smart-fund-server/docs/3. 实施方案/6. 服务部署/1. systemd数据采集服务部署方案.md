...
> 2026-08-19 更新：本文中的多 Python systemd unit 保留为首次 Compose 迁移失败时的回滚基线。
> 当前目标架构已改为 THS Android 继续使用宿主机 systemd，Smart Fund Python 服务、Milvus 和
> etcd 使用 `deployment/docker/compose.production.yml`。宿主机仅新增一个
> `smart-fund-compose.service` 负责开机拉起 Compose 栈。

## 1. 文档定位

本文定义 `smart-fund-server` 在 `119.23.227.187:1113` 服务器上的生产部署方式，作为后续改造
`/home/yuyang/frida-test/smart-fund-server/deployment/deploy_113.sh` 的实施依据。

生产常驻进程由 systemd 分别管理。JetTask 采集 Worker 按底层资源和延迟等级收敛为三个池，
而不是按每类数据源各启动一个进程：

| systemd 服务 | 启动命令 | 职责 |
|---|---|---|
| `smart-fund-api.service` | `python -m src.interfaces.cli api` | FastAPI、Streamable HTTP MCP、Browser Spy、Graph Community Explorer、LLM Proxy |
...
| `smart-fund-worker-ths.service` | `python -m src.interfaces.cli worker --group ths -c 8` | 普通 THS App 协议采集；受 Android 通道容量约束 |
| `smart-fund-worker-ths-sector.service` | `python -m src.interfaces.cli worker --group ths-sector -c 4` | 延迟敏感的高频板块 fragment/reference/signal |
| `smart-fund-worker-general.service` | `python -m src.interfaces.cli worker --group general -c 12` | HTTP 数据源、补采、自选股扫描和研究 Agent 等通用任务 |
| `smart-fund-ths-realtime-stream.service` | `python -m src.interfaces.cli ths-realtime-stream` | THS 长连接行情流；不属于 JetTask Worker |
...
每个 JetTask queue 只能归属于上述一个 Worker 池，禁止重复注册。`http` 与 `internal` 历史池已
合并为 `general`；`ths-sector` 保持独立，防止长耗时 HTTP/Agent 任务挤占高频板块采集。

服务端部署支持 `server-api`、`server-persist`、`server-scheduler`、`server-workers`、
`server-ths-stream` 和 `server-kg` 六个增量组件。一次命中多个组件时只同步一次 Git revision、
执行一次迁移，然后只重启所选进程。共享 application/domain/infrastructure、依赖、schema 或
部署文件变化回退为全部服务端组件。

## 2. 当前问题

原有 `deploy_113.sh` 按旧部署形态工作：
...
flowchart LR
    subgraph Server["119.23.227.187 / 内网 10.168.1.113"]
        ...
        ThsWorker["smart-fund-worker-ths -c 8"]
        SectorWorker["smart-fund-worker-ths-sector -c 4"]
        GeneralWorker["smart-fund-worker-general -c 12"]
    ...
    SectorWorker --> Redis
    GeneralWorker --> Redis
    ThsWorker --> BusinessDB
    SectorWorker --> BusinessDB
    GeneralWorker --> BusinessDB
    GeneralWorker --> API
    GeneralWorker --> Embedding
    GeneralWorker --> Reranker
    CardWorker --> Redis
...
SMART_FUND_MCP_BEARER_TOKEN=<由部署环境注入的随机密钥>
SMART_FUND_MCP_TARGET=prod
SMART_FUND_MCP_ADAPTER_NAME=financial
SMART_FUND_MCP_PUBLIC_URL=http://119.23.227.187:8900/mcp
```
...
Wants=smart-fund-api.service smart-fund-persist.service smart-fund-scheduler.service smart-fund-worker-ths.service smart-fund-worker-ths-sector.service smart-fund-worker-general.service smart-fund-kg-card.service smart-fund-kg-relation.service smart-fund-kg-graph.service
After=network-online.target
...
## 8. `deployment/deploy_113.sh` 改造范围
...
11. `deployment/deploy_113.sh` 的状态、日志和测试命令覆盖七个服务，而不是只检查旧 API 服务。
