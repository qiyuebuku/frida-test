...
|---|---|
| assignments | 归档裁决数组。 |
| new_communities | 本轮需要新建的 Community 定义数组。 |

`assignments` 单项只保留最小裁决字段：

| 字段 | 含义 |
|---|---|
| action | attach_existing / create_new |
| community_id | `attach_existing` 时引用候选 alias；`create_new` 时引用 `new_communities.client_id`。 |
| weight | topic intent 与 community 的归属强度。 |
| confidence | 模型对裁决的确定程度。 |
| reason | 简短说明为什么挂入或新建。 |

`new_communities` 单项只保留：

| 字段 | 含义 |
|---|---|
| client_id | 本次响应内的新建引用 ID，例如 `new_1`。 |
| title | 可复用的父级主题名。 |
| scope | 这个主题未来可承载的信息范围。 |

不再要求 Assignment LLM 输出 `candidate_fit_judgements`、`rejected_candidates`、`maintenance_hints`、`update_mode`、`level`、`title_quality`、`future_coverage`、`covered_subtopics`、`candidate_fit_summary`。这些字段要么由系统推导，要么属于后续 Community 整理/摘要阶段，不应压在逐条归档裁决里。
...
候选判断必须使用“目录归档视角”，而不是“候选摘要是否已经包含当前细节”的视角。只要候选可以作为父级目录承接当前 intent，即使当前细节、主体、风险点、供需状态或项目进展是候选中尚未出现的新材料，也应优先 `attach_existing`。

`create_new` 对应的 `new_communities.title` 必须是可长期复用的父级主题，不能是：
...
当 Assignment LLM 输出 `create_new` 时，必须在 `reason` 中说明：
...
- 校验 `action` / `community_id` / `weight` / `confidence` / `reason` 是否符合 schema。
- 校验 `attach_existing.community_id` 是否来自候选 community。
- 校验 `create_new.community_id` 是否引用 `new_communities.client_id`。
- 校验 `new_communities.title` / `scope` 是否非空。
...
- 不允许把 `create_new` 改成 `attach_existing`。
- 不允许把 `attach_existing` 改成 `create_new`。
- 不允许改写 `new_communities.title`。
- 不允许用父主题规则覆盖 LLM 标题。
...
不能用这些规则改写 LLM 输出的标题或 community_id。
```
...
本阶段先不引入 Community Maintenance。原因是数据高频增量写入时，任何全局或大范围目录整理都会引入额外复杂度和上下文成本。当前优化目标是先让 Assignment 裁决本身稳定、轻量、可观测。

后续如果要处理 L0 过多、相近主题并存、标题层级偏细等问题，应单独设计局部目录维护能力，而不是把 split / merge / rename 塞回逐条 Assignment schema。
