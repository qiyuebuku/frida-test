...
| `kg_relation_discovery` | 单 Card 执行 Probe 召回、rerank、初筛和原文核验；完成该 Card 的正式 Edge、事实投影和语义索引持久化 | Edge 变化事件 |
...
Card 是关系发现的任务分片和持久化检查点。生产调度为每张新增 Card
投递一条独立消息；关系服务即使收到旧格式的多 Card 消息，也必须在每张
Card 核验结束后立即完成以下闭环，再开始下一张 Card：

1. 正式 Edge 当前态写入 PostgreSQL；
2. 受影响 `fact_id` 投影刷新；
3. Card 双视图和 Edge 语义材料同步到 Milvus；
4. 发布对应的图变化事件。

因此大型批次不会再把全部关系结果保留在进程内存中。后续 Card 失败时，
此前已经完成检查点的 Card 结果仍然可见，重试由稳定身份和 upsert 幂等吸收。
...
- 关系发现消息只携带一张 Card；同一 Card 的重复任务由稳定 Pair 和 Edge 身份幂等吸收；
- 兼容旧消息或人工验证产生的多 Card 调用，但每张 Card 必须独立建立持久化检查点；
...
新闻入口和 Card 提取可以使用有界小批次；跨 Chunk 关系发现不把多张 Card
合并为一个生产任务，而是按 Card 分片投递。批量投递只是一组独立消息的
发送优化，不改变每张 Card 独立执行、独立重试和独立持久化的语义。
...
- 关系发现失败隔离到具体 Card；兼容多 Card 调用时，已完成的 Card 检查点不回滚；
