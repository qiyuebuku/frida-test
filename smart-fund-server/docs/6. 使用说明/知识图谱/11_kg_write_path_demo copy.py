...
DEMO_SCENARIO = "weather_community_merge_repro_20260705"
DEMO_TS = "2026-05-20T09:30:00+08:00"
RUN_SESSION_ID = f"kg-write-demo:{DEMO_SCENARIO}:{int(time.time())}"

USE_REAL_FT_NEWS = False
USE_HARDCODED_FT_NEWS = True
FT_NEWS_CANDIDATE_LIMIT = 800
FT_NEWS_RECORD_LIMIT = 4
FT_NEWS_MIN_TEXT_CHARS = 120

VERIFY_QUERY = "最近气象灾害、预警发布和应急响应是否被归并到同一高维主题？"
SEMANTIC_LIMIT = 12
MILVUS_GET_LIMIT = 12
STEP_HEARTBEAT_SECONDS = 15

# 复现 Langfuse 中观察到的 L0 过度拆分场景：
# - ft_news:95281 先创建“自然灾害应急响应”
# - ft_news:98163 又创建“气象灾害预警”
# - ft_news:98164 再创建“气象灾害”
# - ft_news:98965 后续挂入“气象灾害”
#
# 这个 copy 脚本用于验证后续 prompt / merge action 优化是否真实修复问题。
# 默认会清理这些历史错误分裂出的 community，保证复现场景不被现有脏数据污染。
WEATHER_MERGE_REPRO = True
RESET_WEATHER_REPRO_COMMUNITIES = os.getenv("KG_REPRO_RESET_COMMUNITIES", "1").strip().lower() not in {
    "0",
    "false",
    "no",
}
WEATHER_REPRO_RESET_TITLES = {
    "自然灾害应急响应",
    "气象灾害预警",
    "气象灾害",
}
WEATHER_REPRO_FT_NEWS_IDS: tuple[int, ...] = (
    95281,
    98163,
    98164,
    98965,
...
HARDCODED_FT_NEWS_IDS: tuple[int, ...] = (
    *WEATHER_REPRO_FT_NEWS_IDS,
)

DEFAULT_REPRESENTATIVE_FT_NEWS_IDS: tuple[int, ...] = (
    83904,  # A股并购重组市场结构变化
...
async def main() -> None:
    ...
    run_metadata = {
        ...
        "weather_merge_repro": WEATHER_MERGE_REPRO,
        "reset_weather_repro_communities": RESET_WEATHER_REPRO_COMMUNITIES,
        "weather_repro_ft_news_ids": list(WEATHER_REPRO_FT_NEWS_IDS),
    }
    ...
    with langfuse_propagation_context(
        ...
        with langfuse_observation(
            ...
            try:
                ...
                if WEATHER_MERGE_REPRO:
                    outputs["weather_merge_repro_quality"] = _observed_sync_step(
                        "demo.step3_5.weather_merge_repro_quality",
                        step_3_5_evaluate_weather_merge_repro,
                    )
...
async def step_0_6_cleanup_previous_demo_data() -> dict[str, Any]:
    ...
    with get_session(TARGET) as session:
        ...
        repro_reset_community_ids = _weather_repro_community_ids_to_reset(session)
        graph_ids["community_ids"] = list(dict.fromkeys([*graph_ids["community_ids"], *repro_reset_community_ids]))
        ...
        deleted["weather_repro_existing_assignments"] = _delete_count(
            session,
            delete(KnowledgeCommunityAssignment).where(
                KnowledgeCommunityAssignment.adapter_name == ADAPTER,
                KnowledgeCommunityAssignment.community_id.in_(repro_reset_community_ids),
            ),
        )
    ...
    result = {
        ...
        "weather_repro_reset_community_ids": repro_reset_community_ids,
    ...
    pprint(
        {
            ...
            "weather_repro_reset_communities": repro_reset_community_ids,
...
def step_3_5_evaluate_weather_merge_repro() -> dict[str, Any]:
    """评估自然灾害/气象灾害复现场景是否仍然拆出多个 L0。"""

    _section("Step 3.5", "评估气象灾害 community 融合效果")
    source_ids = [f"{DEMO_PREFIX}:ft_news:{news_id}" for news_id in WEATHER_REPRO_FT_NEWS_IDS]
    with get_session(TARGET) as session:
        cards = session.scalars(
            select(KnowledgeCognitiveCard)
            .where(
                KnowledgeCognitiveCard.adapter_name == ADAPTER,
                KnowledgeCognitiveCard.source_id.in_(source_ids),
            )
            .order_by(KnowledgeCognitiveCard.source_id, KnowledgeCognitiveCard.chunk_index)
        ).all()
        assignments = session.scalars(
            ...
            .where(
                KnowledgeCommunityAssignment.adapter_name == ADAPTER,
                KnowledgeCommunityAssignment.cognitive_card_id.in_([row.cognitive_card_id for row in cards]),
            )
            .order_by(KnowledgeCommunityAssignment.created_at, KnowledgeCommunityAssignment.intent_index)
        ).all()
        community_ids = list(dict.fromkeys(row.community_id for row in assignments if row.community_id))
        communities = session.scalars(
            select(KnowledgeGraphCommunity)
            .where(
                KnowledgeGraphCommunity.adapter_name == ADAPTER,
            ...
            .order_by(KnowledgeGraphCommunity.created_at, KnowledgeGraphCommunity.community_id)
        ).all() if community_ids else []

    community_rows = [_community_row(row) for row in communities]
    assignment_rows = [_community_assignment_row(row) for row in assignments]
    weather_like = [
        row
        for row in community_rows
        if _looks_like_weather_disaster_community(row)
    ]
    weather_titles = [str(row.get("title") or "") for row in weather_like]
    split_stage_titles = [
        title
        for title in weather_titles
        if any(token in title for token in ("预警", "应急", "响应"))
    ]
    result = {
        "source_ids": source_ids,
        "card_count": len(cards),
        "assignment_count": len(assignments),
        "assigned_community_count": len(community_rows),
        "assigned_communities": community_rows,
        "assignments": assignment_rows,
        "weather_like_community_count": len(weather_like),
        "weather_like_communities": weather_like,
        "split_stage_titles": split_stage_titles,
        "passed": len(weather_like) <= 1 and not split_stage_titles,
        "expected_after_fix": (
            "所有气象灾害、预警发布、应急响应和后续影响应归入同一个更稳定的父级 L0；"
            "不应同时出现“气象灾害”“气象灾害预警”“自然灾害应急响应”这类平级拆分。"
        ),
    }
    pprint(
        {
            ...
            "weather_like_community_count": result["weather_like_community_count"],
            "weather_titles": weather_titles,
            "split_stage_titles": split_stage_titles,
            ...
            "assignment_sample": assignment_rows[:8],
        },
        sort_dicts=False,
    )
    return result
...
def _weather_repro_community_ids_to_reset(session: Any) -> list[str]:
    if not WEATHER_MERGE_REPRO or not RESET_WEATHER_REPRO_COMMUNITIES:
        return []
    rows = session.scalars(
        select(KnowledgeGraphCommunity).where(
            KnowledgeGraphCommunity.adapter_name == ADAPTER,
            KnowledgeGraphCommunity.title.in_(sorted(WEATHER_REPRO_RESET_TITLES)),
        )
    ).all()
...
def _looks_like_weather_disaster_community(row: dict[str, Any]) -> bool:
    text = json.dumps(
        {
            "title": row.get("title"),
