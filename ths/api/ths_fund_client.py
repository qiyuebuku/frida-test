...
class THSFundClient:
    ...
    def __init__(self, timeout: float = 10.0, use_jsbridge: bool = True, proxy_port: int = 18900):
        """初始化客户端

        Args:
            timeout: HTTP请求超时时间
            use_jsbridge: 是否使用JSBridge代理（推荐True，更稳定）
            proxy_port: JSBridge代理端口（默认18900）
        """
        ...
        # JSBridge代理配置
        self.use_jsbridge = use_jsbridge
        self.proxy_url = f"http://localhost:{proxy_port}"
        if use_jsbridge:
            # 创建专门用于JSBridge的同步客户端（JSBridge代理是同步的）
            import requests
    ...
    async def _proxy_request(self, path: str, method: str = "POST",
        ...
        """通过代理转发请求（用于 /rz/ 路径的 API，需要 App 级认证）

        如果启用JSBridge，将使用JSBridge代理；否则使用旧的18900代理
        """
        if self.use_jsbridge:
            # 通过JSBridge代理调用
            headers = {
            ...
            # 解析body为params字典（针对POST请求）
            if body:
                if content_type == "application/x-www-form-urlencoded":
                    # form-encoded格式：key1=val1&key2=val2
                    ...
                    import json as json_lib
                    try:
                        params = json_lib.loads(body) if isinstance(body, str) else body
                    except:
                        params = {"body": body}
                else:
                    # 其他格式直接作为字符串传递
            ...
            result = await loop.run_in_executor(
                ...
                self._call_jsbridge_sync,
                method,
                f"{self.TRADE_BASE_URL}{url_path}",  # 使用分离后的路径
                params,
                headers,
                True,  # needToken
                "none"  # K5type
    ...
    async def buy_fund(self, fund_code: str, amount: float, use_wallet: bool = True,
        ...
        init_data = init_resp.get("data", {}) if self.use_jsbridge else init_resp.get("data", init_resp)
        ...
        acct_data = acct_resp.get("data", {}) if self.use_jsbridge else acct_resp.get("data", {})
        ...
        # Step 4: 获取短信验证状态
        # 如果使用JSBridge，刷新认证参数获取最新的key5（同步方法需要在executor中运行）
        if self.use_jsbridge:
            import asyncio
            loop = asyncio.get_event_loop()
            auth = await loop.run_in_executor(None, self._refresh_trade_auth)
        ...
        # JSBridge格式：{"errorCode": 0, "result": {"code": "0000", "singleData": {...}}}
        if self.use_jsbridge:
            result_obj = checksms_resp.get("result", {})
            code = result_obj.get("code", "")
            if code not in ("0000", ""):
                raise ValueError(f"checksms失败 [{code}]: {result_obj.get('message')}")
            trade_info_seq = result_obj.get("singleData", {}).get("tradeInfoSeq", "") or result_obj.get("tradeInfoSeq", "")
        else:
        ...
        buy_data = buy_resp.get("data", {}) if self.use_jsbridge else buy_resp.get("data", buy_resp)
        return {
            "fund_code": fund_code,
            "fund_name": fund_name,
            "amount": money,
    ...
    async def sell_fund(self, fund_code: str, share_vol: str = None,
        ...
        # JSBridge格式：{"errorCode": 0, "result": {"code": "0000", "data": {...}}}
        if self.use_jsbridge:
    ...
    async def cancel_order(self, app_sheet_serial_no: str, password: str = None) -> dict:
        ...
        if self.use_jsbridge:
            list_data = list_resp.get("result", {}).get("singleData", {}).get("data", [])
        else:
        ...
        detail_data = detail_resp.get("data", {}) if self.use_jsbridge else detail_resp.get("data", detail_resp)
