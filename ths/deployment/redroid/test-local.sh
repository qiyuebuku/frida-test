...
usage() {
    cat >&2 <<'EOF'
usage: test-local.sh [--mode collector|trade] [--base-image IMAGE]
...
"$SCRIPT_DIR/add-instance.sh" --name "$NAME" --mode "$MODE" \
    --adb-port 5559 --http-port 49690 --image "$image" \
    --ready-timeout "$READY_TIMEOUT" "${trade_args[@]}"
echo "local verification passed: mode=$MODE, http=http://127.0.0.1:49690"
